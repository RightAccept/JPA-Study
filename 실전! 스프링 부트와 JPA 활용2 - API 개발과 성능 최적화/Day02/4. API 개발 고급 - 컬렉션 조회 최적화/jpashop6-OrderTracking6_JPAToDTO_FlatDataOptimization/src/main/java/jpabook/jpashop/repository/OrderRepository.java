...
//                .setFirstResult(1)    // collection fetch에서 페이징을 시도하면, DB에서 데이터를 퍼와서 메모리에서 페이징을 하게된다
                                        //  - DB에서는 join으로 인해 컬렉션에 해당하는 테이블의 데이터 수만큼 출력 수가 늘어나게 되므로, DB에서는 페이징이 불가능하다
//                .setMaxResults(100)   // 만약 데이터가 1만개라면, 1만개의 데이터를 DB에서 메모리로 가져와서, 페이징을 하게 되므로 서버에 과부하가 걸리게 된다
                .getResultList();
    }
